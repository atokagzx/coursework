...
struct process_msgs
{
    ...
    static void received_video(TgBot::Bot &bot, TgBot::Message::Ptr &message) {
        ...
        if (should_save) {
            std::ofstream out("database/" + std::to_string(message->chat->id) + "/" + std::to_string(hash) + ".mp4", std::ios::binary);
...
int main() {
    ...
    bot.getEvents().onNonCommandMessage([&bot](TgBot::Message::Ptr message) {
            if (message->video) {
                ...
                long id = message->chat->id;
                bool does_exist = filesystem_database::does_user_exist(id);
                bot.getApi().sendMessage(message->chat->id, does_exist ? "Чтобы добавить описание к видео, выбери контекстное меню видеоролика и используй опцию reply(ответить)" : "Используй команду </setname [Имя]> чтобы представиться");
            }
        });
